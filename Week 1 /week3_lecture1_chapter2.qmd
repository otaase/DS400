...
```{r}
ggplot(data = fake_news, aes(x = type, fill = title_has_excl)) + geom_bar()
```
...
```{r}
prior_probability_has_excl <- 0.12
```
...
```{r}
(prior_probability_article_fake * likelihood_article_fake_has_exc) / prior_probability_has_excl
```
...
3.  Marginal prior probability that person uses the word pop
